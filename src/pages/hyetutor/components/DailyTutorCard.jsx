...
// Entry card on the HyeTutor page
// 4 states: no plan / pending / in-progress / completed
...
// ============================================================
// HELPERS — read cache + plan directly (same keys as hook)
// ============================================================
...
export function DailyTutorCard({ onOpen }) {
  ...
  useEffect(() => {
    ...
    // Re-read if the modal finished and wrote to cache
    const onStorage = (e) => {
  ...
  // Re-read on remount / re-render when parent triggers open
  useEffect(() => {
    if (loading) return
    setSession(readTodaySession())
  ...
  // -----------------------------------------------------------
  // Base styles shared by all states
  // -----------------------------------------------------------
  const cardStyle = {
    padding: 'var(--space-5)',
    cursor: 'pointer',
  ...
  if (!planTopic && !session) {
    return (
      <section
        ...
        style={cardStyle}
        onClick={() => {
          // Route to study plan setup — parent decides, but we give them a hook
          // by passing null topic so HyeTutorPage knows to bounce
          onOpen?.({ needsPlan: true })
        }}
      >
        {headerRow}
        <div
  ...
  // -----------------------------------------------------------
  // STATE 2 — Pending (plan exists, no session today)
  // -----------------------------------------------------------
  if (planTopic && !session) {
    return (
      <section
      ...
      >
        ...
        <div style={{ marginBottom: 'var(--space-3)' }}>
          ...
          >
            {planTopic.subject}
            {planTopic.hours ? ` · ~${Math.round(planTopic.hours * 60)} min` : ''}
          </div>
        </div>
  ...
  // -----------------------------------------------------------
  // STATE 3 — In progress (started but not completed)
  // -----------------------------------------------------------
  if (session && session.status === 'in_progress') {
    ...
    return (
      ...
      >
        ...
        <button
          className="btn btn-warning flex-center"
          style={{ width: '100%' }}
          onClick={(e) => {
            e.stopPropagation()
            onOpen?.({ action: 'resume' })
          }}
        >
            ...
            >
              ...
              {session.reflection?.feeling ? (
                <>
                  ...
                  />
                  <span>
                    {session.reflection.feeling === 'clear'
                      ? '😄'
                      : session.reflection.feeling === 'okay'
                        ? '😐'
                        : '😕'}{' '}
                    {session.reflection.feeling}
                  </span>
