...
export const ai = {
  ...
  dailyTutor: {
    generateLesson: async (payload) => {
      if (USE_MOCK) {
        await delay(MOCK_DELAYS.slow)
        const examLabel = (payload.exam_type || 'jamb').toUpperCase()
        return {
          success: true,
          ...
          lesson: {
            ...
            sections: [
              ...
              },
              {
                ...
                body: `At its core, ${payload.topic} describes a relationship between the quantities involved. The key is to see what stays constant and what changes.\n\nFor ${examLabel}, they love testing this in multi-step questions — so focus on the mechanism, not just the formula.`,
              },
              {
                heading: 'Exam-style example',
                body: `A typical ${examLabel} question will give you two known quantities and ask for a third. Identify what you have, pick the right relationship, and solve step by step.\n\nWatch out for unit conversions — they cost marks every year.`,
              },
              {
